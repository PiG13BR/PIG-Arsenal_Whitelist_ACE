...
if !(toLower (headgear player) in PIG_arsenalAllowed) then {
    _removedItems pushBack (headgear player);
    removeHeadgear player;
};
if !(toLower (goggles player) in PIG_arsenalAllowed) then {
    _removedItems pushBack (goggles player);
    removeGoggles player;
};
if !(toLower (uniform player) in PIG_arsenalAllowed) then {
    _removedItems pushBack (uniform player);
    removeUniform player;
};
if !(toLower (vest player) in PIG_arsenalAllowed) then {
    _removedItems pushBack (vest player);
    removeVest player;
};
if (!(toLower (backpack player) in PIG_arsenalAllowed) && ((backpack player) != _backpack)) then {
...
} forEach (((_items arrayIntersect _items) - PIG_arsenalAllowed) select {!([_x] call PIG_fnc_isRadio)});
...
} forEach ((_items arrayIntersect _items) - PIG_arsenalAllowed);

// Check weapons stored in inventory containers
{
    if (!isNull _x) then {_removedItems append ([_x] call PIG_fnc_checkWeaponCargo);};
...
} forEach (_items - PIG_arsenalAllowed);
...
if !(_removedItems isEqualTo []) exitWith {
    [_removedItems] spawn {
        ...
        [format ["Found %1 at player %2", _removedItems, name player], "BLACKLIST"] remoteExecCall ["PIG_fnc_log", 2];
