/*
    ...
    Description:
        Removes weapons not found in PIG_arsenalAllowed from a cargo space.
...

{
    ...
    _components = _weapon call PIG_fnc_getWeaponComponents;
    ...
    if ((toLower ([_x] call BIS_fnc_baseWeapon)) in PIG_arsenalAllowed) then {
        ...
        if !(toLower _muzzle in _presetAttachments) then {
            if (toLower _muzzle in PIG_arsenalAllowed) then {
        ...
        if !(toLower _pointer in _presetAttachments) then {
            if (toLower _pointer in PIG_arsenalAllowed) then {
        ...
        if !(toLower _optic in _presetAttachments) then {
            if (toLower _optic in PIG_arsenalAllowed) then {
        ...
        if !(toLower _bipod in _presetAttachments) then {
            if (toLower _bipod in PIG_arsenalAllowed) then {
        ...
        if (_magazineClass != "") then {
            if (toLower _magazineClass in PIG_arsenalAllowed) then {
        ...
        if (_magazineGLClass != "") then {
            if (toLower _magazineGLClass in PIG_arsenalAllowed) then {
